...
func _input(event):
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		if Input.is_key_pressed(KEY_F1):
			...
			var zombie = load("res://unit/Zombie/basic_zombie/buckethead_basic_zombie.tscn").instantiate()
			get_tree().current_scene.add_child(zombie)
			zombie.global_position = get_global_mouse_position()
...
func take_damage(source : Node, value : int = 20,):
	...
	if hp <= 0:
		hp= 10
		if source: if source.master.has_method("kill_a_zombie"): source.master.kill_a_zombie()
...
func _on_hitpoint_area_entered(area: Area2D) -> void:
	...
	if body.get_groups().has("ally_projectile"):
		take_damage(body, body.damage)
		body.reduce_pierce()
	if body.has_method("remove_zmog"):
		body.remove_zmog()
...
func _on_hitpoint_area_shape_exited(area_rid: RID, area: Area2D, area_shape_index: int, local_shape_index: int) -> void:
	if !area: return
	var body = area.get_parent()
	if body.has_method("_add_zmog"):
		body._add_zmog()
