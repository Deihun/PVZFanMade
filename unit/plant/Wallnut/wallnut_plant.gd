...
func _heal_when_im_planted_on_my_tile():
	$plant_health_management_behaviour.heal_maxhealth_percentage(100.0)
...
func _on_tier_2b_empower_body_entered(body: Node2D) -> void:
	if body.is_in_group("plant"):
		...
		if max_health: 
			...
			if $EvolutionSenderSupportBehavior._tier1a_obtain: 
				max_health.heal($plant_health_management_behaviour.armor*10)
			if $EvolutionSenderSupportBehavior._tier1b_obtain: 
				$plant_health_management_behaviour.gain_armor(0.4)




func _on_tallnut_blocking_body_entered(body: Node2D) -> void:
	if body.is_in_group("zombie") and !body.is_in_group("testing"):
		body.position.x += 150
		if body.has_node("_tallnut_block"): body._tallnut_block
